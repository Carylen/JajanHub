...
import { useSendOtp, useSession, useVerifyOtp } from '@jajanhub/api';
...
/**
 * Owns the phone + WhatsApp-OTP login flow (BRIEF: gate Profil/Pesanan
 * Aktif/Langganan/Bayar, guest browsing stays open) and renders a single
 * global sheet (mobile) or modal (desktop) — same "hook owns logic, view
 * renders it" split as usePaymentScreen/useQueueScreen. `useSession()` (React
 * Query, backed by the mock's authStore) is the sole source of truth for
 * `isLoggedIn`, so there's no second, possibly-divergent client store.
 */
