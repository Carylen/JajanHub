...
import { BottomSheet, Button, Chip, cn } from '@jajanhub/ui';
...
/** Post-order rating, presented as a bottom sheet modal (BRIEF §4/§5). */
export function RatingModal({ open, onClose, merchantName, orderCode, onSubmit }: RatingModalProps) {
  ...
  return (
    <BottomSheet open={open} onClose={onClose} label="Beri rating pesanan">
      ...
      <Button variant="ghost" fullWidth className="text-faint pt-3.5" onClick={() => onSubmit(0, [], '')}>
        Nanti aja
      </Button>
    </BottomSheet>
  );
