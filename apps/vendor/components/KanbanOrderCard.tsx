'use client';
import type { VendorOrder } from '@jajanhub/api';
...
export function KanbanOrderCard({ order, onAdvance, onReject }: KanbanOrderCardProps) {
  ...
  const canReject = !!cfg && (order.status === 'baru' || order.status === 'masak');

  return (
    <div
      className={cn(
        'rounded-2xl p-[15px] shadow-[0_4px_12px_rgba(35,24,15,.05)] animate-[slideCard_.3s_ease] border',
        order.priority ? 'bg-[#FBF8FF] border-2 border-[#C9B0FF]' : 'bg-white border-[#F1E7DC]',
      )}
    >
      <div className="flex items-center gap-[11px]">
        <div
          className="flex-none w-[46px] h-[46px] rounded-[13px] flex flex-col items-center justify-center text-white leading-none"
          style={{ background: order.priority ? 'linear-gradient(135deg,#A879FF,#7A3BF5)' : 'linear-gradient(135deg,#FFB870,#FF7A1A)' }}
        >
